...
DATA_DIR = Path(__file__).parent / "graphrag-workspace" / "output"
INPUT_DIR = Path(__file__).parent / "graphrag-workspace" / "input"
...
# Use modern FastAPI lifespan instead of deprecated on_event
from contextlib import asynccontextmanager

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Startup
    print("Starting up Resume RAG API...")
    success = load_data()
    if not success:
        print("WARNING: Failed to load some data. Functionality may be limited.")
    yield
    # Shutdown
    print("Shutting down Resume RAG API...")

app = FastAPI(title="Resume RAG API", version="1.0.0", lifespan=lifespan)

# Re-add CORS middleware after recreating app
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "http://localhost:3000"],  # Vite default port
    allow_credentials=True,
